...
int HEAP_ALLOC(UNK* SIZE)
{
    ...
    {
        CHUNK_LIST_MERGE((CHUNK*)CHUNK_BASE->TEMP_CHUNKS, ((CHUNK*)CHUNK_BASE->FREE_CHUNKS));
        assert(CHUNK_BASE->FREE_CHUNKS = CHUNK_BASE->TEMP_CHUNKS);

        for (UNK i = 0; i < *(UNK*)CHUNK_BASE->FREE_CHUNKS; i++)
        {
            ...
            if(HEAP_BASE->SIZE >= (UNK*)(&HEAP_BASE->SIZE_BYTEWISE))
            {
